...
<label for="requirements">Requirements or product brief</label><input id="requirements" name="requirements" type="file" accept=".txt,.md,.json,.pdf,.docx"><small>TXT, Markdown, JSON, PDF, or DOCX. Approved JSON contracts can drive a scoped release gate.</small>
...
form.addEventListener('submit',async e=>{e.preventDefault();button.disabled=true;show('Submitting assessment…');try{const body=new FormData(form);if(!body.get('requirements')?.size)body.delete('requirements');const response=await fetch('/api/jobs',{method:'POST',body});const data=await response.json();if(!response.ok)throw new Error(data.detail||'Unable to start');const id=data.job_id;for(;;){await new Promise(r=>setTimeout(r,2500));const check=await fetch('/api/jobs/'+id),job=await check.json();show('<strong>Job '+id+'</strong>\n\n'+(job.recent_log||job.status));if(job.status==='COMPLETE'){const cls=job.decision==='INSUFFICIENT EVIDENCE'?'INSUFFICIENT':job.decision;show('<div class="decision '+cls+'">'+job.decision+'</div><p>'+JSON.stringify(job.summary,null,2)+'</p><a href="/api/jobs/'+id+'/report" style="color:#69e6c1">Download QA Director report</a>');break}if(job.status==='FAILED')throw new Error(job.error||'Assessment failed')}}}catch(error){show('<span style="color:#ff718b">'+error.message+'</span>')}finally{button.disabled=false}});
</script></body></html>`;
